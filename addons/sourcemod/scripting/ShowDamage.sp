...
{
	g_cvAllowForBots = CreateConVar("sm_show_damage_allowforbots", "1", "Allow bots to create hit particles (NOTE: Only will be visible when spectating a bot)", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvReconnectPlayer = CreateConVar("sm_show_damage_enableplayerreconnect", "1", "Enable this to force new players to reconnect to server, so they will see particle effects without need of waiting next map", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvCommands = CreateConVar("sm_show_damage_commandnames", "sm_fortnitehits;sm_hits;sm_damage;sm_fortnite;", "Set custom names here for toggle damage display command, don't add to many commands as it may overflow buffer. (NOTE: Write command names with \"sm_\" prefix, and don't use ! or any other symbol except A-Z and 0-9 and underline symbol \"_\", also server needs to be restarted to see changes!)", FCVAR_NONE);
	g_cvDistance = CreateConVar("sm_show_damage_distance", "50.0", "Distance between victim player and damage numbers (NOTE: Make that value lower to prevent numbers show up through the walls)", FCVAR_NONE, true, 0.0);
	g_cvPermission = CreateConVar("sm_show_damage_flag", "", "Set any flag here if you want to restrict use of that plugin only to certain flag (NOTE: Leave it empty to allow anyone to use this plugin)", FCVAR_NONE);

	g_cvPermission.AddChangeHook(Cvar_Permission_ChangeHook);
	LoadTranslations("ShowDamage.phrases");

	RegConsoleCommands();
	g_hCookie = RegClientCookie("sm_show_damage_state", "Is showing damage disabled/enabled for a specific client", CookieAccess_Protected);
